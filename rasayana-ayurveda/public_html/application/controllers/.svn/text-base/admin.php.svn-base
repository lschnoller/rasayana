...
class Admin extends Controller {
    ...
    function newsletter($do = '', $id = 0, $u = 0) {
        if (!$this->admin_functions->isAdminLoggedIn()) {
            redirect(base_url() . 'admin');
        }
        $data['list'] = FALSE;
        ...
        if ($do == 'form') {
            $header['section'] = 'Edit Newsletter';
            if ($id == 0)
                $header['section'] = 'Add Newsletter';
            $this->load->view('admin/header', $header);
            /*             * ********************************* Newsletter add-modify ******************************** */
            $this->form_validation->set_rules('title', 'Title', 'trim|required');
            $this->form_validation->set_rules('main_article', 'Main Article', 'trim|required');
            $this->form_validation->set_rules('main_desci', 'Main Desci', 'trim|required');
            $this->form_validation->set_rules('url', 'URL', 'trim|required|prep_url|min_length[11]');
            ...
            $imageUpload = $this->upload->do_upload('image_url');
            if ($imageUpload) {
                $image = $this->upload->data();
                $values['image_url'] = $image['file_name'];
            }
            if ($id == 0)
                $upload = $imageUpload;

            if ($this->form_validation->run() == FALSE || !$upload) {
                $data['id'] = $id;
                if ($u != 0)
                    $data['error'] = $this->upload->display_errors();
                $data['values'] = $this->admin_functions->getNewsletter($id);
                $data['message'] = '';
            }else {
                $values['title'] = $this->admin_functions->getPostValue('title');
                $values['main_article'] = $this->admin_functions->getPostValue('main_article');
                $values['url'] = $this->admin_functions->getPostValue('url');
                $values['main_desci'] = $this->admin_functions->getPostValue('main_desci');

                $put = $this->admin_functions->putNewsletter($values, $id);
                if ($put) {
                    $data['message'] = '<div id="success">New Newsletter successfuly updated</div>';
                    $data['list'] = $this->admin_functions->getNewsletterList();
                } else {
                    $data['id'] = $id;
                    $data['message'] = '<div id="error">Error on action, please try again!!</div>';
                }
            }
            /*             * ******************************* Newsletter add-modify eof ****************************** */
        } elseif ($do == 'del' && is_numeric($id)) {
            $header['section'] = 'Newsletter';
            $this->load->view('admin/header', $header);
            /*             * *********************************Newsletter del ********************************** */
            $del = $this->admin_functions->deleteNewsletter($id);
            $data['message'] = $del;
            $data['list'] = $this->admin_functions->getNewsletterList();
            /////////// Newsletter del eof
        } else {
            $header['section'] = 'List of Newsletter';
            $this->load->view('admin/header', $header);
            /*             * ************************ Newsletter list ********************************** */
            $data['message'] = '';
            $data['list'] = $this->admin_functions->getNewsletterList();
    ...
    function newsletter_code($id = 0) {
        $data['news'] = $this->admin_functions->getNewsletterView($id);
        $this->load->view('admin/newsletter_code', $data);
    }

    function newsletter_prev($id = 0) {
        $data['news'] = $this->admin_functions->getNewsletterView($id);
        $this->load->view('admin/newsletter-view', $data);
    }

    function newsletter_new($id = 0) {
        $this->load->view('admin/newsletter-new-view');
    }
    ...
    function newsletter_sub($do = '', $me = 0, $id = 0, $u = 0) {
        if (!$this->admin_functions->isAdminLoggedIn()) {
            redirect(base_url() . 'admin');
        }
        $data['list'] = FALSE;
        ...
        $data['id'] = $id;
        if ($do == 'form') {
            $data['section'] = 'Edit Newsletter Sub';
            if ($id == 0)
                $data['section'] = 'Add Newsletter Sub';
            /*             * ********************************* Newsletter Sub add-modify ******************************** */
            $this->form_validation->set_rules('title', 'Title', 'trim|required');
            $this->form_validation->set_rules('desci', 'Main Desci', 'trim|required');
            $this->form_validation->set_rules('url', 'URL', 'trim|required|prep_url|min_length[11]');
            ...
            if ($this->form_validation->run() == FALSE || !$upload) {
                if ($u != 0)
                    $data['error'] = $this->upload->display_errors();
                $data['values'] = $this->admin_functions->getNewsletterSub($id);
                $data['message'] = '';
            }else {
                $values['title'] = $this->admin_functions->getPostValue('title');
                $values['url'] = $this->admin_functions->getPostValue('url');
                $values['desci'] = $this->admin_functions->getPostValue('desci');
                $values['main'] = $me;
                $put = $this->admin_functions->putNewsletterSub($values, $id);
                if ($put) {
                    $data['message'] = '<div id="success">New Newsletter Sub successfuly updated</div>';
                    $data['list'] = $this->admin_functions->getNewsletterSubList($me);
                } else {
                    $data['message'] = '<div id="error">Error on action, please try again!!</div>';
                }
            }
            /*             * ******************************* Newsletter Sub add-modify eof ****************************** */
        } elseif ($do == 'del' && is_numeric($id)) {
            $data['section'] = 'Newsletter Sub';
            /*             * *********************************Newsletter Sub del ********************************** */
            $del = $this->admin_functions->deleteNewsletterSub($id);
            $data['message'] = $del;
            $data['list'] = $this->admin_functions->getNewsletterSubList($me);
            /////////// Newsletter del eof
        } else {
            $data['section'] = 'List of Newsletter Sub';
            /*             * ************************ Newsletter v list ********************************** */
            $data['message'] = '';
            $data['list'] = $this->admin_functions->getNewsletterSubList($me);
            /*             * ********************************* Newsletter Sub list eof ********************************** */
        }
        $this->load->view('admin/newsletter_sub', $data);
    }
